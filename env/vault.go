...
var _ env.Provider = (*VaultProvider)(nil)
...
func (v *VaultProvider) Get(name string) (string, merry.Error) {
	...
	if secret == nil {
		return "", env.NameNotSet.Prepend("vault env provider: get").Append(name)
	}
	...
	if r == "" {
		return "", env.NameEmpty.Prepend("vault env provider: get").Append(name)
	}
