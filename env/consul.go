...
var _ env.Provider = (*ConsulProvider)(nil)

type kvMonitor struct {
	ch         chan<- env.Value
	init       bool
	lastIndex  uint64
	lastResult env.Value
	del        bool
}
...
func (p *ConsulProvider) Get(name string) (string, merry.Error) {
	...
	if kvp == nil {
		return "", env.NameNotSet.Prepend("consul env provider: get").Append(name)
	}

	r := string(kvp.Value)
	if r == "" {
		return "", env.NameEmpty.Prepend("consul env provider: get").Append(name)
	}
...
func (p *ConsulProvider) GetInt(name string) (int, merry.Error) {
	...
	if kvp == nil {
		return 0, env.NameNotSet.Prepend("consul env provider: get int").Append(name)
	}
...
func (p *ConsulProvider) GetBool(name string) (bool, merry.Error) {
	...
	if kvp == nil {
		return false, env.NameNotSet.Prepend("consul env provider: get bool").Append(name)
	}
...
func (p *ConsulProvider) monitorLoop() {
	...
	for !p.shouldStop() {
		...
		for _, kvp := range kvps {
			...
			val := env.Value{Name: key, Value: string(kvp.Value)}
