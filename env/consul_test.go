...
func TestConsulProviderGetMissing(t *testing.T) {
	...
	assert.Equal(t, "", r)
	assert.Error(t, err)
	assert.True(t, merry.Is(err, env.NameNotSet))
	kvg.AssertGetCalledOnceWith(t, defaultKey, nil)
}
...
func TestConsulProviderMonitorChange(t *testing.T) {
	...
	expectedV1 := env.Value{Name: defaultKey, Value: newVal}
	expectedV2 := env.Value{Name: defaultKey, Value: string(defaultVal)}
...
func TestConsulProviderMonitorIndexReset(t *testing.T) {
	...
	expectedNew := env.Value{Name: defaultKey, Value: newVal}
	expectedDefault := env.Value{Name: defaultKey, Value: string(defaultVal)}
...
func TestConsulProviderMonitorMultipleMonitor(t *testing.T) {
	...
	expectedNew := env.Value{Name: defaultKey, Value: newVal}
	expectedDefault := env.Value{Name: defaultKey, Value: string(defaultVal)}
	expectedAny := env.Value{Name: anyKey, Value: newVal}
...
func TestConsulProviderMonitorRevenant(t *testing.T) {
	...
	e1 := env.Value{}
	e2 := env.Value{Name: defaultKey, Value: string(defaultVal)}
